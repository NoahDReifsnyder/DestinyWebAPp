...
const organizeForm = document.querySelector("#organize-weapons");
...
organizeForm?.addEventListener("submit", async (event) => {
  ...
  const confirmed = window.confirm(
    "This will refresh your inventory, lock every keeper in the vault, and " +
      "unlock every review candidate. It may take several minutes. Continue?",
  );
  if (!confirmed) {
    return;
  }
...
async function pollProgress() {
  while (true) {
    const response = await fetch("/cleaner/weapons/organize/status", {
      headers: { Accept: "application/json" },
      cache: "no-store",
    });
    ...
    if (["complete", "partial", "failed"].includes(job.status)) {
      ...
      window.location.assign(`/cleaner/weapons?${key}=${encodeURIComponent(message)}`);
      return;
    }
...
async function resumeActiveProgress() {
  try {
    ...
    // A page reload must remain usable even if a transient status poll fails.
  }
}
