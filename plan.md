...
| 2026-08-08 | Loadout capability refactor | Awaiting user composition and live verification | Loadout behavior is exposed through distinct route-independent modules for inspection, individual saves/revisions, activity sets, previews, durable execution, atomic Bungie actions, and loadout-owned SQLite persistence. All high-level signatures use `(functions, user_id, *, ...)`; all atomic writes use `(bungie, access_token, *, ...)`; slot indices are consistently zero-based. The application factory again registers the loadout services and routes from one construction point. Existing saved loadouts and durable operation history load successfully with clean SQLite integrity and foreign keys; no Bungie mutation was sent during refactoring. |
